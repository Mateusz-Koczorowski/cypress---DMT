describe('Take a simple order with files (no cart)', () => {
  ...
  it('Chceck if it is HP', () => {
    cy.url().should('eq', 'https://stage.drukomat.co/');
  })

  it('Open and find group randomly', () => {
    cy.contains("a", "Produkty").click();
    cy.url('https://stage.drukomat.co/produkty').should('eq', 'https://stage.drukomat.co/produkty')
    cy.get('.btn.btn-blue.jsCookiesAccept').should('be.visible').click(); 
    var products = ['Gazetki reklamowe', 'Ulotki', 'Banery', 'Kalendarze', 'Notesy', 'Arkusze plano', 'Teczki ofertowe', 'Wizytówki']
  ...
  it('Open and find product randomly', () => {
